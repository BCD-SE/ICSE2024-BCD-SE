...
private:
  ...
  std::unordered_set<llvm::Value*> uniqueBrSet;
  dg::LLVMDependenceGraph *dg;
  time::Span divide_time;
  time::Span analysis_time;
  time::Span check_time;
  int check_nums;
  typedef std::set<llvm::Value*> branchTy;
  typedef std::map<std::pair<llvm::Value*, unsigned char>, 
          std::pair<bool, std::set<std::pair<llvm::Value*, unsigned char>>> > brTy;
  std::vector<llvm::Instruction*> initModule;
  std::vector<std::vector<std::string>> branchSet;
  unsigned int errorCount;
  unsigned int validPath;
  std::map<ref<Expr>, std::vector<ref<Expr> > > valueMap;
  bool searched;
  typedef dg::LLVMNode* depTy;
  std::map<dg::LLVMNode*, std::set<depTy> > brToValMap;
  unsigned int staticReStates;
  unsigned int dynamicReStates;
  int reverse_limit;
  int states_limit;
  ...
  /// \invariant \ref addedStates and \ref removedStates are disjoint.0.0@
...
public:
  ...
  void runFunctionAsMain(llvm::Function *f, int argc, char **argv,
                         char **envp, dg::LLVMDependenceGraph *DG, std::vector<llvm::Instruction*> initM) override;
  ...
  bool compareStates(ExecutionState &state1, ExecutionState &state2);

  bool checkStates(ExecutionState &s);

  dg::LLVMNode *findSubNode(dg::LLVMDependenceGraph *dg, llvm::Value *val);

  void computeBranchDependence(dg::LLVMDependenceGraph *dg, int step);

  void devideBranch(ExecutionState &state);

  bool judgeDependence(ExecutionState &state, dg::LLVMNode *node, dg::LLVMNode *n_node);

  bool findInstruction(llvm::Value* i, std::vector<llvm::Instruction*> m);
