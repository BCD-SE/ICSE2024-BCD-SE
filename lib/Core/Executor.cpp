...
Executor::Executor(LLVMContext &ctx, const InterpreterOptions &opts,
    ...
    : Interpreter(opts), interpreterHandler(ih), searcher(0),
      ...
      // errorCount(0), validPath(0), searched(false), redundantStates(0){
      errorCount(0), staticReStates(0), dynamicReStates(0), check_nums(0){
 ...
 //             errs() << ("skip CEX cache HIT on false!\n") ;
  ...
  if (res==Solver::True) {
    if (!isInternal) {
      if (pathWriter) {
        current.pathOS << "1";
        if(current.brSet.find(current.currentBr) != current.brSet.end()){
          current.brSet[current.currentBr] = '1';
          // current.brSet.erase(current.brSet.find(current.currentBr));
        }
      }
    }
    ...
    if (!isInternal) {
      if (pathWriter) {
        current.pathOS << "0";
        if(current.brSet.find(current.currentBr) != current.brSet.end()){
          current.brSet[current.currentBr] = '0';
          // current.brSet.erase(current.brSet.find(current.currentBr));
        }
      }
    }
    ...
    if (pathWriter) {
      ...
      if (!isInternal) {
        ...
        trueState->pathOS << "1";
        if(trueState->brSet.find(trueState->currentBr) != trueState->brSet.end()){
          trueState->brSet[trueState->currentBr] = '1';
        }
        falseState->pathOS << "0";
        if(falseState->brSet.find(falseState->currentBr) != falseState->brSet.end()){
          falseState->brSet[falseState->currentBr] = '0';
        }
      }
    }
    if (symPathWriter) {
...
//    errs() << "Adding true and falste constraints \n";
    ...
    // judge the redudency of the true states
    const auto divide_start_time = klee::time::getWallTime();
    devideBranch(*trueState);
    falseState->branchSet = trueState->branchSet;
    time::Span divide_elapsed_time(time::getWallTime() - divide_start_time);
    divide_time += divide_elapsed_time;

    const auto check_start_time = klee::time::getWallTime();
    if(!checkStates(*trueState) && states.size() >= states_limit){
      check_nums++;
      if(std::find(removedStates.begin(), removedStates.end(), trueState) == removedStates.end()){
        staticReStates++;
        removedStates.push_back(trueState);
      }
      time::Span check_elapsed_time(time::getWallTime() - check_start_time);
      check_time += check_elapsed_time;
      return StatePair(0, falseState);
    }

    if(!checkStates(*falseState) && states.size() >= states_limit){
      check_nums++;
      if(std::find(removedStates.begin(), removedStates.end(), falseState) == removedStates.end()){
        staticReStates++;
        removedStates.push_back(falseState);
      }
      time::Span check_elapsed_time(time::getWallTime() - check_start_time);
      check_time += check_elapsed_time;
...
void Executor::executeInstruction(ExecutionState &state, KInstruction *ki) {
  ...
  case Instruction::Br: {
    ...
    if (bi->isUnconditional()) {
      ...
      // uniqueBrSet.emplace(i);
      // FIXME: Find a way that we don't have this hidden dependency.
      assert(bi->getCondition() == bi->getOperand(0) &&
             "Wrong operand index!");

        
      if(findInstruction(i, initModule)){
        state.currentBr = i;
        state.brSet.insert(std::make_pair(i, '0'));
      }
...
void Executor::run(ExecutionState &initialState) {
  ...
  const auto analysis_start_time = klee::time::getWallTime();
  /*Compute the dependence information*/
  computeBranchDependence(dg, 1);

  time::Span analysis_elapsed_time(time::getWallTime() - analysis_start_time);
  analysis_time += analysis_elapsed_time;

  errs() << "Br information: " << brToValMap.size() << "\n";
...
//  searcher = new PendingSearcher(searcher, pendingStateSearcher, this);
  ...
  while (!states.empty() && !searcher->empty() && !haltExecution) {
    ...
    // llvm::errs() << state.pendingConstraint.isNull() << "\n";
    updateStates(&state);
  }

  //   
  std::uint32_t ah, ch, dh;
  std::uint8_t am, as, cm, cs, dm, ds;
  std::tie(ah,am,as) = time::seconds(analysis_time.toSeconds()).toHMS();
  std::tie(dh,dm,ds) = time::seconds(divide_time.toSeconds()).toHMS();
  std::tie(ch,cm,cs) = time::seconds(check_time.toSeconds()).toHMS();
  std::stringstream analysisInfo;
  analysisInfo << "Analysis: " << analysis_time.toSeconds() <<"s "
            << std::setfill('0') << std::setw(2) << ah
            << ':'
            << std::setfill('0') << std::setw(2) << +am
            << ':'
            << std::setfill('0') << std::setw(6) << +as
            << '\n'
            << "Divede: " << divide_time.toSeconds() <<"s "
            << std::setfill('0') << std::setw(2) << dh
            << ':'
            << std::setfill('0') << std::setw(2) << +dm
            << ':'
            << std::setfill('0') << std::setw(6) << +ds
            << '\n'
            << "Check: " << check_time.toSeconds() <<"s "
            << std::setfill('0') << std::setw(2) << ch
            << ':'
            << std::setfill('0') << std::setw(2) << +cm
            << ':'
            << std::setfill('0') << std::setw(6) << +cs
            << '\n';
  analysisInfo << "Check nums: " << check_nums << "\n";
  interpreterHandler->getInfoStream() << analysisInfo.str();
  interpreterHandler->getInfoStream().flush();

  std::stringstream stats;
  stats << "\n";
  stats << "KLEE: done: redundant paths = "
        << staticReStates << "\n";
  // stats << "KLEE: done: dynamic redundant paths = "
  //       << dynamicReStates << "\n";
  ...
  if (useColors)
    llvm::errs().changeColor(llvm::raw_ostream::GREEN,
                             ...
                             /*bg=*/false);
  llvm::errs() << stats.str();
  interpreterHandler->getInfoStream() << stats.str();
  interpreterHandler->getInfoStream().flush();

  delete searcher;
  searcher = 0;

  // errs() << "Unique branch size is: " << uniqueBrSet.size() << "\n";
...
void Executor::runFunctionAsMain(Function *f,
         ...
         std::vector<llvm::Instruction*> initM) {
      
    
  dg = DG;
  initModule = initM;
  reverse_limit = interpreterOpts.reverseLimit;
  states_limit = interpreterOpts.statesLimit;
...
// compute the branch dependence information to the brToValMap[node, {{curNode, dstNode, T/F, visited} ... }]
void Executor::computeBranchDependence(dg::LLVMDependenceGraph *dg, int step){
  std::queue<dg::LLVMDependenceGraph *> m_map;
  std::set<dg::LLVMDependenceGraph *> visited;
  m_map.push(dg);
  visited.insert(dg);
  int flag = true;
  int reverse = 0;
  while (flag){     
    flag = false;                       
    while(!m_map.empty()){
      dg::LLVMDependenceGraph *temp = m_map.front();
      m_map.pop();
      for(auto I = temp->begin(), E = temp->end(); I != E; ++I){
        // reverse every node
        if (llvm::Instruction *Inst = llvm::dyn_cast<llvm::Instruction>(I->first)){
          if(Inst->getOpcode() == Instruction ::Br && !cast<BranchInst>(Inst)->isUnconditional()){ 
            //add the current node if not exit
            if(brToValMap.find(I->second) == brToValMap.end()){ 
              flag = true;
              std::set<depTy> tempSet;
              tempSet.insert(I->second);
              brToValMap.insert(std::make_pair(I->second, tempSet));
            }
            // add the node that control denpendeced with the current node
            dg::LLVMBBlock* BB = I->second->getBBlock();
            for(auto S : BB->controlDependence()){ 
              dg::LLVMNode* firstNode = S->getFirstNode();
              if(brToValMap[I->second].count(firstNode) == 0){
                flag = true;
                brToValMap[I->second].emplace(firstNode);
              } 
            }
            // add the node that data denpendeced with the current node
            for(auto dataD = I->second->data_begin(), dataE = I->second->data_end(); 
                     dataD != dataE; ++dataD){
              dg::LLVMBBlock* BB = (*dataD)->getBBlock();
              dg::LLVMNode* firstNode = BB->getFirstNode();  
              if(brToValMap[I->second].count(firstNode) == 0){
                flag = true;
                brToValMap[I->second].emplace(firstNode);
              }
            ...
            // traverse every node in the set
            for(auto iterB = brToValMap[I->second].begin(), iterE = brToValMap[I->second].end(); 
                     iterB != iterE; ++iterB){
              auto node = *iterB;
              // add the controlD nodes
              dg::LLVMBBlock* BB = node->getBBlock();
              for(auto S : BB->controlDependence()){ 
                dg::LLVMNode* firstNode = S->getFirstNode();  
                if(brToValMap[I->second].count(firstNode) == 0){
                  flag = true;
              ...
              // add the dataD nodes
              for(auto dataD = node->data_begin(), dataE = node->data_end(); 
                    dataD != dataE; ++dataD){             
               if(brToValMap[I->second].count(*dataD) == 0){
                 flag = true;
                 brToValMap[I->second].emplace(*dataD);
               }
              }
              // add the used nodes
              for(auto useD = node->use_begin(), useE = node->use_end(); 
                       useD != useE; ++useD){  
                if((*useD)->getBBlock()->getKey() == node->getBBlock()->getKey()){           
                  if(brToValMap[I->second].count(*useD) == 0){
                    flag = true;
                    brToValMap[I->second].emplace(*useD);
                  }
                }
              }
            }
        ...
        if(reverse > reverse_limit){ 
          continue; 
        }
        if(I->second->hasSubgraphs()){
          for(auto subdg : I->second->getSubgraphs()){
            if(visited.find(subdg) == visited.end()){
              reverse++;
              m_map.push(subdg);
              visited.insert(subdg);
            }
          }
        }
      }
...
// judge two node have dependence, if two nodes has dependence then return true
bool Executor::judgeDependence(ExecutionState &state, dg::LLVMNode *node, dg::LLVMNode *n_node){
  bool findFlag = false;
  for(auto nodeT = brToValMap[node].begin(), nodeE = brToValMap[node].end();
           nodeT != nodeE; ++nodeT){
    if(brToValMap[n_node].find(*nodeT) != brToValMap[n_node].end()){
      findFlag = true;
      break;
    }
  }
...
// divide the branches
void Executor::devideBranch(ExecutionState &state){
  for(auto br : state.brSet){
    // judge the br if exits in the branchSet
    bool findFlag = false;
    if(state.branchSet.find(br.first) != state.branchSet.end()){
      findFlag = true;
    }
    for(auto S : state.branchSet){
      for(auto s : S.second){
        if(s == br.first){
          findFlag = true;
        }
    ...
    // judge the br if exit in brToValMap
    bool nodeFlag = true;
    auto node = findSubNode(dg, br.first);
    if(node != NULL && brToValMap.find(node) == brToValMap.end()){
      nodeFlag = false;
    }
    if(findFlag || !nodeFlag) continue;
    if(state.branchSet.empty()){
      std::set<llvm::Value*> tempSet;
      state.branchSet.insert(std::make_pair(br.first, tempSet));
      continue;
    }
    bool dealed = false;
    for(auto &S : state.branchSet){
      if(dealed) break;
      auto l_node = findSubNode(dg, S.first);
      auto r_node = findSubNode(dg, br.first);
      if(l_node != NULL && r_node != NULL && judgeDependence(state, l_node, r_node)){
        continue;
      }else{
        if(S.second.empty()){
          S.second.insert(br.first);
          dealed = true;
          break;
        }
        bool flag = false;
        for(auto s : S.second){
          auto m_node = findSubNode(dg, s);
          if(m_node != NULL && judgeDependence(state, m_node, r_node)){
            flag = true;
            break;
          }
        }
...
// check the current state if redudent: if redudent retrun false
bool Executor::checkStates(ExecutionState &s){
  for(auto S : s.branchSet){
    auto node = S.first;
    for(auto n : S.second){
      if(s.brSet[node] != s.brSet[n]){
        return false;
      }
    }
  }
...
dg::LLVMNode *Executor::findSubNode(dg::LLVMDependenceGraph *dg, llvm::Value *val){
  std::queue<dg::LLVMDependenceGraph *> m_maps;
  m_maps.push(dg);
  while(!m_maps.empty()){
    dg::LLVMDependenceGraph *temp = m_maps.front();
    m_maps.pop();
    for(auto I = temp->begin(), E = temp->end(); I != E; ++I){
      if(val == I->first){
          return I->second;
...
bool Executor::findInstruction(llvm::Value* i, std::vector<llvm::Instruction*> m){
